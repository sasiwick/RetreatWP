...
const handleBulk = async (actionType) => {
    if (selectedUsers.length === 0) return;

    const userdata = {
        userIDs: selectedUsers,
        status_label: actionType  // <-- Dynamic approve/deny
    };

    try {
        setLaoding(true);
        const response = await update_user_status('update-user-bulk', userdata);
        ...
        if (response.message === 'Success') {
            ...
            if (page > newPageCount) {
                ...
                fetchPendingUsers(); // reload current page
            }
    ...
    return (
        ...
        <TableContainer className="pending_users_tbl_container usersTable" component={Paper} sx={{ overflowX: 'auto' }}>
        ...
        usersdata.map((row) => (
          <TableRow key={row.ID}>
            {dynamicColumns.map((col) => {
              ...
              // Default render (email, registration date, etc.)
              return (
                <TableCell key={col.key}>
                  {row[col.key] || <em>-</em>}
                </TableCell>
