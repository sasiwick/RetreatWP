...
const Approved_Users = () => {
    ...
    const fetchApprovedUsers = async () => {
        ...
        try{
            setLoading(true);
            const response = await axios.get(`${NUARestAPI.get_approved_users+NUARestAPI.permalink_delimeter}page=${page}&limit=${rowsPerPage}&search=${search}`,{ 
                headers: {
...
const handleSelectClick = (userId) => {
  ...
  const handleBulkDeny = async () => {
      ...
      try {
          ...
          if (response.message === 'Success') {
            ...
            if (page > newPageCount) {
                ...
                fetchApprovedUsers(); // reload current page
            }
          } else {
              toast.error(
                  __("Failed to update user status", "new-user-approve"),
    ...
    let iconDeny = (
      ...
      <path d="M19 14.9927L17.4921 16.4994L19 18.0073L18.0073 19L16.4994 17.4921L14.9927 19L14 18.0073L15.5067 16.4994L14 14.9927L14.9927 14L16.4994 15.5067L18.0073 14L19 14.9927Z" fill="white"/>
      </svg>
    );
...
const availableKeys = usersdata.length > 0 ? Object.keys(usersdata[0]) : [];
    ...
    return (
        ...
        <div className = "approved_users_list">
        
        <div className = "all_users_list_header">
            <h2 className='users_list_title'>  {__('Approved Users', 'new-user-approve') }</h2>
        ...
        <TableContainer
  className="approved_users_tbl_container usersTable"
  component={Paper}
  sx={{ overflowX: 'auto' }}
>
  <Table sx={{ minWidth: 650 }}>
    {/* Render header only if we have columns */}
      ...
      {loading ? (
        ...
        [...Array(10)].map((_, index) => (
          <TableRow key={index}>
            {dynamicColumns.length > 0 ? (
              ...
              // If columns aren't ready yet, show full-width skeleton
              <TableCell colSpan={5}>
                <Skeleton variant="text" width="100%" height={20} />
              </TableCell>
            )}
        ...
        usersdata.map((row) => (
          <TableRow key={row.ID}>
            {dynamicColumns.map((col) => {
                ...
                return (
                  ...
                  >
                    {action_status(row.nua_status).map((status) => (
                      <div className="nua-deny" key={status}>
                        <IconButton
                          style={{ paddingLeft: "0" }}
              ...
              // Default render for all other columns
              return (
                <TableCell key={col.key}>
                  {row[col.key] || <em>-</em>}
                </TableCell>
        ...
        <TableRow>
          <TableCell colSpan={dynamicColumns.length || 5}>
            <div
              className="user-list-empty approved-user-empty-list"
